...
func birthdayCakeCandles(candles []int32) int32 {
	...
	tallestCandleMap := make(map[int32]int32)

	for _, candle := range candles {
		if _, found := tallestCandleMap[candle]; found {
			tallestCandleMap[candle]++
		} else {
			tallestCandleMap[candle] = 1
		}
	}

	var bigger int32

	for _, value := range tallestCandleMap {
		if value > bigger {
			bigger = value
		}
	}
